...
* Computer learning what the monitor is, by reading out its EDID (Extended Display Identification Data).
* Computer learning what adjustables the monitor has, like Luminance (*some* may call it *"brightness"*), Contrast, Input Source... Audio Bass, OSD Language, Bottom Corner Flare, Flesh tone enhancement, Trapezoid, Flat panel sub-pixel layout... etc.
* Computer adjusting the adjustables occasionally.
...
* the distance from these traces to their ground(s? if this is not the top layer) is important,
* there should be no other copper somewhat "close" to the traces, or it will affect the impedance (or you *can* have copper close, but then it matters how close you have it, and it will affect the width and distance between the traces).
...
* Have a solid ground under the traces without any cuts or such. Many pictures on the Internet will illustrate that there is a magnetic field between the trace and the ground under it; for high speed signals a PCB is just a wave guide, and if the ground under the trace is cut, then your wave can't pass without going around the gap, and just don't make your wave spread out all around unnecessarily.
    * Nice and simple is to have the signal on an outermost layer, and the ground for it on the layer immediately below it
...
* Don't just put your signal through lots of vias if it can be avoided
    * If you put it through a via, keep in mind where its ground is in relation; remember, copper is a wave guide in this case. Add one or several ground vias around the signal via
* If a high-speed signal has to go above/below another signal, cross them at 90 degrees
* Don't add copper pours if there is no reason for them; don't add ground pours "for good luck"; never have floating copper pours. It is important to have copper on your board to prevent it from physically bending once it reaches a certain size; then having some floating copper dots or squares works well, as long as they are small enough to not become antennas.
...
* A diffpair should be at least ~3 times the track width away from other copper on the same layer, and at the same time at least ~1.2 mm away from other copper on the same layer.
...
* I connected all the grounds together. If there is common-mode noise on one of the pairs, it would negatively affect other pairs since the return paths for common mode noise are now all connected together at the midpoint.
...
* I did not round the corners! (at least this mistake is now fixed on Github)

### Interception

<img src="ddcci-logic-analyzer.jpg" class="illustration" />
...
* [Debian on a Framework Laptop 13 with a monitor connected, idle](/ddcci-capture-idle.csv)
...
Miniaturization is a trend that's hard to escape, for example, when your husband asks for a smaller device for his already cluttered desk. So I later replaced the DDC/CI injector box with the [DDC/CI dongle](https://github.com/parport0/ddcci_dongle). It does the same thing. But theoretical signal integrity is, hopefully, better.
...
Work in progress hardware devices are on my Github, for example: [displayport\_aux\_checkpoint](https://github.com/parport0/displayport_aux_checkpoint).

<img src="ddcci-devices.jpg" class="illustration" />
